...
module Graphlyte
  module Editors
    # Editor to collect fragment spread information
    # within a document
    # used for Validation
    class CollectFragmentSpreads
      ...
      def editor
        ...
        Editor
          .new
          .on_fragment_spread { |spread, _action| @fragment_spreads << spread.name }
      ...
      def get_duplicates(spread_array)
        grouped = spread_array.each_with_object({}) do |spread, memo|
          memo[spread] = (memo[spread] || 0) + 1
        end
        grouped.select { |_k, v| v > 1 }.keys
      end
